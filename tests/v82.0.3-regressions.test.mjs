...
assert.match(html, /styles\/legacy\.css\?v=82\.0\.3/);
assert.match(html, /styles\/premium-v82\.0\.3\.css\?v=82\.0\.3/);
assert.doesNotMatch(html, /styles\/(tokens|foundation|home|components|motion|responsive|v78\.4-critical)\.css/);
assert.match(html, /boot-v82\.0\.3\.mjs\?v=82\.0\.3/);
assert.match(html, /__NINOU_BOOT_WATCHDOG__/);
assert.match(html, /history\.replaceState/);

assert.match(boot, /const NINOU_VERSION = "82\.0\.3"/);
assert.match(boot, /const MIN_SPLASH_MS = 1500;/);
assert.match(boot, /visual-guard-v82\.0\.3/);
assert.match(core, /const NINOU_RUNTIME_VERSION = "82\.0\.3"/);
assert.match(core, /const NINOU_FAMILY_SCOPE_VERSION = "82\.0\.3-premium-consolidated"/);
assert.match(ux, /const UX_VERSION = "82\.0\.3"/);
assert.match(stability, /const STABILITY_VERSION = "82\.0\.3"/);
...
assert.match(premiumCss, /v82\.0\.3 — relógio Céu Vivo/);
...
assert.match(sw, /const APP_VERSION = "82\.0\.3"/);
assert.match(sw, /const STYLE_MODULES = \["legacy", "premium-v82\.0\.3"\]/);
