...
const [html, boot, core, ux, stability, premiumCss, visualGuard, sw, build, vercel, daySky, nightSky] = await Promise.all([
  ...
  readFile(new URL("js/boot-v81.0.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-core-v81.0.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-ux-v81.0.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-stability-v81.0.0.mjs", root), "utf8"),
  readFile(new URL("styles/premium-v81.0.0.css", root), "utf8"),
  readFile(new URL("js/runtime/visual-guard-v81.0.0.mjs", root), "utf8"),
...
const bootBarrier = html.indexOf('classList.add("ninou-booting", "ninou-v8100")');
...
assert.match(html, /styles\/legacy\.css\?v=81\.0\.0/);
assert.match(html, /styles\/premium-v81\.0\.0\.css\?v=81\.0\.0/);
assert.doesNotMatch(html, /styles\/(tokens|foundation|home|components|motion|responsive|v78\.4-critical)\.css/);
assert.match(html, /boot-v81\.0\.0\.mjs\?v=81\.0\.0/);
assert.match(html, /__NINOU_BOOT_WATCHDOG__/);
assert.match(html, /history\.replaceState/);

assert.match(boot, /const NINOU_VERSION = "81\.0\.0"/);
assert.match(boot, /const MIN_SPLASH_MS = 1500;/);
assert.match(boot, /visual-guard-v81\.0\.0/);
assert.match(core, /const NINOU_RUNTIME_VERSION = "81\.0\.0"/);
assert.match(core, /const NINOU_FAMILY_SCOPE_VERSION = "81\.0\.0-premium-consolidated"/);
assert.match(ux, /const UX_VERSION = "81\.0\.0"/);
assert.match(stability, /const STABILITY_VERSION = "81\.0\.0"/);
...
assert.match(premiumCss, /v81\.0\.0 — relógio Céu Vivo/);
...
assert.match(sw, /ninou-v80-1-3-ajustes-orbita-menu/);
assert.match(sw, /const APP_VERSION = "81\.0\.0"/);
assert.match(sw, /const STYLE_MODULES = \["legacy", "premium-v81\.0\.0"\]/);
...
const premiumSize = (await stat(new URL("styles/premium-v81.0.0.css", root))).size;
...
console.log("Regressões v81.0.0 validadas: céu claro solar, noite cósmica, órbita local, menu + e estabilidade preservados.");

// v81.0.0: ajustes solicitados após validação visual.
