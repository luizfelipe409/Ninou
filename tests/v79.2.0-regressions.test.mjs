...
const [html, boot, core, ux, stability, premiumCss, visualGuard, sw, build, vercel] = await Promise.all([
  ...
  readFile(new URL("js/boot-v79.2.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-core-v79.2.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-ux-v79.2.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-stability-v79.2.0.mjs", root), "utf8"),
  readFile(new URL("styles/premium-v79.2.0.css", root), "utf8"),
  readFile(new URL("js/runtime/visual-guard-v79.2.0.mjs", root), "utf8"),
...
const bootBarrier = html.indexOf('classList.add("ninou-booting", "ninou-v792")');
...
assert.match(html, /styles\/legacy\.css\?v=79\.2\.0/);
assert.match(html, /styles\/premium-v79\.2\.0\.css\?v=79\.2\.0/);
...
assert.match(boot, /visual-guard-v79\.2\.0/);

assert.match(core, /const NINOU_RUNTIME_VERSION = "79\.2\.0"/);
assert.match(core, /const NINOU_FAMILY_SCOPE_VERSION = "79\.2\.0-premium-consolidated"/);
...
assert.match(ux, /const UX_VERSION = "79\.2\.0"/);
assert.match(stability, /const STABILITY_VERSION = "79\.2\.0"/);
...
assert.match(sw, /const APP_VERSION = "79\.2\.0"/);
assert.match(sw, /const STYLE_MODULES = \["legacy", "premium-v79\.2\.0"\]/);
assert.match(sw, /visual-guard-v79\.2\.0/);
...
const premiumSize = (await stat(new URL("styles/premium-v79.2.0.css", root))).size;
...
console.log("Regressões v79.2.0 validadas: menu do botão mais, amamentação respirada, Perfil com safe area e navegação fixa, órbita animada e folha sem vazamento.");
