...
const releaseVersion = JSON.parse(await readFile(new URL("package.json", root), "utf8")).version;
const releaseVersionPattern = releaseVersion.replace(/\./g, "\\.");
const [html, boot, core, adminRuntime, adminService, firebaseService, ux, stability, appCss, adminCss, appShell, actionLauncher, recordSheet, visualGuard, sw, build, vercel, daySky, nightSky] = await Promise.all([
  readFile(new URL("index.html", root), "utf8"),
  readFile(new URL("js/bootstrap.mjs", root), "utf8"),
  readFile(new URL("js/app-core.mjs", root), "utf8"),
  readFile(new URL("js/admin-runtime.mjs", root), "utf8"),
  readFile(new URL("js/services/admin-service.js", root), "utf8"),
  readFile(new URL("js/services/firebase-service.js", root), "utf8"),
  readFile(new URL("js/ui/ux-runtime.mjs", root), "utf8"),
  readFile(new URL("js/runtime/stability.mjs", root), "utf8"),
  readFile(new URL("styles/app.css", root), "utf8"),
  readFile(new URL("styles/admin.css", root), "utf8"),
  readFile(new URL("js/ui/app-shell.mjs", root), "utf8"),
  readFile(new URL("js/ui/action-launcher.js", root), "utf8"),
  readFile(new URL("js/ui/record-sheet.js", root), "utf8"),
  readFile(new URL("js/runtime/visual-guard.mjs", root), "utf8"),
...
// Aliases preservam toda a cobertura de regressão visual após a consolidação
// das antigas camadas em uma única folha e um único shell funcional.
const premiumCss = appCss;
const focusedFlowCss = appCss;
const customerReadyCss = appCss;
const webInteractionCss = appCss;
const menuParityCss = appCss;
const mobileExperienceCss = appCss;
const menuParityJs = appShell;
const mobileExperienceJs = appShell;

const headEnd = html.indexOf("</head>");
const bootBarrier = html.indexOf('classList.add("ninou-booting", "ninou-launch-base")');
...
assert.match(html, new RegExp(`styles/app\\.css\\?v=${releaseVersionPattern}`));
assert.doesNotMatch(html, /styles\/(?:admin-v|admin-web-v|legacy|premium-v|focused-flow-v|customer-ready-v|web-interaction-stability-v|web-mobile-)\S*\.css/);
assert.doesNotMatch(html, /<script[^>]+admin-runtime\.mjs/);
assert.equal((html.match(/<link[^>]+rel="stylesheet"/g) || []).length, 1, "A web deve carregar uma única folha comum.");
assert.doesNotMatch(html, /web-mobile-(?:menu-parity|experience)|interaction-stability/);
assert.doesNotMatch(html, /styles\/(tokens|foundation|home|components|motion|responsive|v78\.4-critical)\.css/);
assert.match(html, new RegExp(`bootstrap\\.mjs\\?v=${releaseVersionPattern}`));
assert.match(html, /__NINOU_BOOT_WATCHDOG__/);
assert.match(html, /history\.replaceState/);

assert.match(boot, new RegExp(`const NINOU_VERSION = "${releaseVersionPattern}"`));
assert.match(boot, /const MIN_SPLASH_MS = 1500;/);
assert.match(boot, /runtime\/visual-guard\.mjs/);
assert.match(core, new RegExp(`const NINOU_RUNTIME_VERSION = "${releaseVersionPattern}"`));
assert.match(core, new RegExp(`const NINOU_FAMILY_SCOPE_VERSION = "${releaseVersionPattern}-launch-base"`));
assert.match(core, new RegExp(`const ADMIN_STYLESHEET_HREF = "\\./styles/admin\\.css\\?v=${releaseVersionPattern}"`));
assert.match(core, new RegExp(`const ADMIN_RUNTIME_HREF = "\\./admin-runtime\\.mjs\\?v=${releaseVersionPattern}"`));
...
assert.match(adminRuntime, /services\/admin-service\.js/);
...
assert.match(appCss, /Estabilidade de interações/);
...
assert.match(appCss, /Experiência mobile e live wallpaper/);
assert.match(boot, /ui\/app-shell\.mjs/);
...
assert.match(mobileExperienceCss, /\.subscription-access-card h2/);
assert.match(mobileExperienceJs, /function ensureLiveWallpaper\(\)/);
assert.match(mobileExperienceJs, /function ensureDeleteDialog\(\)/);
assert.match(mobileExperienceJs, /function upgradeInitialRoutineFlow\(\)/);
assert.match(mobileExperienceJs, /Somente visualização/);
assert.doesNotMatch(html, /data-start-mode="now"/);
...
assert.match(sw, /ninou-\$\{APP_VERSION\}-launch/);
assert.doesNotMatch(sw, /admin-runtime\.mjs\?v=\$\{APP_VERSION\}/);
assert.match(sw, new RegExp(`const APP_VERSION = "${releaseVersionPattern}"`));
assert.match(sw, /styles\/app\.css\?v=\$\{APP_VERSION\}/);
assert.match(sw, /day-sky\.svg/);
assert.match(sw, /night-sky\.svg/);
assert.match(build, /"assets", "audio", "icons"/);
assert.match(build, /const publicEntries = \[/);
assert.match(build, /"styles", "js", "assets"/);
assert.match(build, /await cp\(join\(root, entry\), join\(output, entry\), \{ recursive: true \}\)/);
assert.doesNotMatch(build, /premium-v82|web-mobile-menu-parity|web-mobile-experience/);
assert.doesNotMatch(build, /admin-web-v82/);
assert.match(build, /packageJson\.version/);
...
const appCssSize = (await stat(new URL("styles/app.css", root))).size;
const adminSize = (await stat(new URL("styles/admin.css", root))).size;

assert.ok(appCssSize < 950 * 1024, "A folha consolidada deve permanecer abaixo de 950 KB.");
...
console.log("Regressões de lançamento validadas: céu vivo, órbita, menus, fluxos e estabilidade preservados.");
