...
if (productionFiles.length && !publicRootFiles.has("styles/admin-v82.0.0.css")) failures.push("CSS administrativo ausente do pacote de produção");
...
if (premiumImportant > 1980) failures.push(`Autoridade premium usa !important em excesso: ${premiumImportant}`);
const legacyCss = await readFile(join(root, "styles/legacy.css"), "utf8");
const legacyImportant = (legacyCss.match(/!important/g) || []).length;
if (legacyImportant > 4860) failures.push(`Camada comum usa !important em excesso: ${legacyImportant}`);
...
if (!core.includes("if (appAdmin) ensureAdminStylesheet()")) failures.push("Núcleo não restringe o CSS administrativo ao admin global");
...
if (totalImportant > 7700) failures.push(`Cascata comum usa !important em excesso: ${totalImportant}`);
...
if (totalCss >= 820 * 1024) failures.push(`CSS comum ainda está grande demais: ${(totalCss/1024).toFixed(1)} KB`);
const adminCssSize = (await stat(join(root, "styles/admin-v82.0.0.css"))).size;
if (adminCssSize >= 120 * 1024) failures.push(`CSS administrativo ainda está grande demais: ${(adminCssSize/1024).toFixed(1)} KB`);
for (const name of [...cssModules, ...conditionalCssModules]) {
  const css = await readFile(join(root, `styles/${name}.css`), "utf8");
  if (/\{\s*\}/.test(css)) failures.push(`Bloco CSS vazio retornou a styles/${name}.css`);
}

console.log(`Ninou v82.0.0: ${files.length} arquivos, ${scripts.length} scripts, CSS comum ${(totalCss/1024).toFixed(1)} KB + admin condicional ${(adminCssSize/1024).toFixed(1)} KB, !important comum ${totalImportant}, admin ${adminImportant}.`);
if (failures.length) {
