...
const failures = [];
const cssModules = ["legacy", "premium-v79.2.0"];
const required = [
  "index.html", "styles.css", "sw.js", "manifest.webmanifest", "firestore.rules", "vercel.json",
  ...cssModules.map((name) => `styles/${name}.css`),
  "js/boot-v79.2.0.mjs", "js/ninou-core-v79.2.0.mjs", "js/ninou-ux-v79.2.0.mjs",
  "js/ninou-consistency-v79.2.0.mjs", "js/ninou-stability-v79.2.0.mjs",
  "js/runtime/architecture-v79.2.0.mjs", "js/runtime/diagnostics-v79.2.0.mjs", "js/runtime/visual-guard-v79.2.0.mjs",
...
for (const file of scripts.filter((file) => /js\/(?:core|repositories|runtime)\//.test(relative(root, file)) && !file.endsWith("diagnostics-v79.2.0.mjs") && !file.endsWith("visual-guard-v79.2.0.mjs"))) {
...
if (!html.includes("boot-v79.2.0.mjs?v=79.2.0")) failures.push("Boot v79.2.0 não está ligado ao index.html");
for (const module of cssModules) if (!html.includes(`styles/${module}.css?v=79.2.0`)) failures.push(`CSS ${module} não está ligado ao HTML`);
...
if (!sw.includes('const STYLE_MODULES = ["legacy", "premium-v79.2.0"]')) failures.push("Service Worker não declara a autoridade visual revisada");
if (!sw.includes('const APP_VERSION = "79.2.0"')) failures.push("Service Worker não está na v79.2.0");
...
const premiumCss = await readFile(join(root, "styles/premium-v79.2.0.css"), "utf8");
const premiumImportant = (premiumCss.match(/!important/g) || []).length;
if (premiumImportant > 1300) failures.push(`Autoridade premium usa !important em excesso: ${premiumImportant}`);
for (const selector of ["client-family-member-avatar", "record-sheet-open .bottom-bar", "quick-observations-panel", "premium-new-episode-button", "record-types", "weight-sparkline svg", "event-meta-primary", "diaryChipsMoreButton", "paint-order: normal"]) {
...
console.log(`Ninou v79.2.0: ${files.length} arquivos, ${scripts.length} scripts, CSS total ${(totalCss/1024).toFixed(1)} KB, !important premium ${premiumImportant}.`);
if (failures.length) {
