...
const required = [
  ...
  "js/boot-v78.2.0.mjs", "js/ninou-core-v78.2.0.mjs", "js/ninou-ux-v78.2.0.mjs",
  "js/ninou-consistency-v78.2.0.mjs", "js/ninou-stability-v78.2.0.mjs",
  "js/runtime/architecture-v78.2.0.mjs", "js/runtime/diagnostics-v78.2.0.mjs",
...
for (const file of scripts.filter((file) => /js\/(?:core|repositories|runtime)\//.test(relative(root, file)) && !file.endsWith("diagnostics-v78.2.0.mjs"))) {
...
if (!html.includes("boot-v78.2.0.mjs?v=78.2.0")) failures.push("Boot v78.2.0 não está ligado ao index.html");
for (const module of cssModules) if (!html.includes(`styles/${module}.css?v=78.2.0`)) failures.push(`CSS ${module} não está ligado ao HTML`);
...
if (premiumImportant > 800) failures.push(`Camada premium usa !important em excesso: ${premiumImportant}`);
...
console.log(`Ninou v78.2.0: ${files.length} arquivos, ${scripts.length} scripts, CSS total ${(totalCss/1024).toFixed(1)} KB, !important premium ${premiumImportant}.`);
if (failures.length) {
