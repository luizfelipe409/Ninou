...
const [html, boot, core, stability, homeCss, foundationCss, componentsCss, motionCss, sw, build] = await Promise.all([
  readFile(new URL("index.html", root), "utf8"),
  readFile(new URL("js/boot-v78.3.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-core-v78.3.0.mjs", root), "utf8"),
  readFile(new URL("js/ninou-stability-v78.3.0.mjs", root), "utf8"),
...
const bootBarrier = html.indexOf('classList.add("ninou-booting", "ninou-v783")');
...
for (const file of ["legacy", "tokens", "foundation", "home", "components", "motion", "responsive"]) {
  assert.match(html, new RegExp(`styles/${file}\\.css\\?v=78\\.3\\.0`));
}
assert.match(html, /boot-v78\.3\.0\.mjs\?v=78\.3\.0/);
assert.doesNotMatch(html, /78\.2\.0|ninou-v782/);
...
assert.match(sw, /ninou-v78-3-0-premium-consolidated/);
assert.match(sw, /const APP_VERSION = "78\.3\.0"/);
...
console.log("Regressões v78.3.0 validadas: boot, retomada, sessão, órbita, atalhos, navegação e formulário.");
