...
const ADMIN_STYLESHEET_HREF = "./styles/admin.css?v=83.0.2";
const ADMIN_RUNTIME_HREF = "./admin-runtime.mjs?v=83.0.2";
...
const NINOU_RUNTIME_VERSION = "83.0.2";
...
const NINOU_FAMILY_SCOPE_VERSION = "83.0.2-launch-base";
  ...
  try {
    ...
    if (inviteResult) {
      inviteResult.textContent = error?.code === "permission-denied"
        ? "Sem permissão para criar convite. Publique as regras Firestore de produção e confirme que está logado com luizfelipe.dasilva@gmail.com."
  ...
  try {
    ...
    if (!options.silent && loginHelper) {
      loginHelper.textContent = error?.code === "permission-denied"
        ? "Sem permissão para aceitar convite. Publique as regras Firestore de produção e confirme se o convite é para este e-mail."
        : getFirebaseErrorMessage(error);
    }
    return false;
  } finally {
...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("/sw.js?v=83.0.2", { updateViaCache: "none" }).then((registration) => {
