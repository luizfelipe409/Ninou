const NINOU_VERSION = "79.0.0";
const LEGACY_REPAIR_KEY = "ninou:pwa-legacy-cleanup:v79.0.0";
...
async function waitForStyleSheets() {
  const links = [...document.querySelectorAll('link[rel="stylesheet"][href*="79.0.0"]')];
...
async function bootNinou() {
  ...
  window.__NINOU_BOOT_PROMISE__ = (async () => {
    ...
    try {
      const architectureModule = await withTimeout(import(`./runtime/architecture-v79.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Arquitetura do aplicativo");
    ...
    await withTimeout(import(`./ninou-core-v79.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Núcleo do aplicativo");

    const layerResults = await Promise.allSettled([
      withTimeout(import(`./ninou-ux-v79.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Experiência de uso"),
      withTimeout(import(`./ninou-consistency-v79.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Consistência visual"),
      withTimeout(import(`./ninou-stability-v79.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Estabilidade"),
      withTimeout(import(`./runtime/diagnostics-v79.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Diagnóstico"),
      withTimeout(import(`./runtime/visual-guard-v79.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Guarda visual"),
    ]);
