...
const NINOU_RUNTIME_VERSION = "82.0.1";
...
const NINOU_FAMILY_SCOPE_VERSION = "82.0.1-premium-consolidated";
...
function prepareVisibleContextForAccount(user = cloudUser) {
  ...
  /*
    v82.0.1 — retomada sem tela fantasma:
  ...
  try {
    ...
    if (inviteResult) {
      inviteResult.textContent = error?.code === "permission-denied"
        ? "Sem permissão para criar convite. Publique as regras Firestore da v82.0.1 e confirme que está logado com luizfelipe.dasilva@gmail.com."
  ...
  try {
    ...
    if (!options.silent && loginHelper) {
      loginHelper.textContent = error?.code === "permission-denied"
        ? "Sem permissão para aceitar convite. Publique as regras Firestore da v82.0.1 e confirme se o convite é para este e-mail."
        : getFirebaseErrorMessage(error);
    }
    return false;
  } finally {
...
function createOrbitEvent(event, active = false, position = eventPosition(event.start)) {
  ...
  button.innerHTML = `<i class="care-icon">${config.icon}</i>`;
...
function createOrbitCluster(group) {
  ...
  button.innerHTML = `<i class="orbit-cluster-icon care-icon">${config.icon}</i><span class="orbit-cluster-count" aria-hidden="true">${eventList.length}</span>`;
...
function renderTimeline() {
  ...
  if (lastCard) {
    ...
    lastCard.innerHTML = `
      <i class="mark care-icon ${latestConfig.arcType}">${latestConfig.icon}</i>
      <div>
  ...
  orbitClusterList.innerHTML = orderedEvents.map((rawEvent) => {
    ...
    return `
      <article class="cluster-card">
        <i class="cluster-icon care-icon ${config.arcType}">${config.icon}</i>
        <div>
...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("/sw.js?v=82.0.1", { updateViaCache: "none" }).then((registration) => {
