const NINOU_VERSION = "82.1.3";
const LEGACY_REPAIR_KEY = "ninou:pwa-legacy-cleanup:v82.1.3";
...
async function bootNinou() {
  ...
  window.__NINOU_BOOT_PROMISE__ = (async () => {
    ...
    try {
      const architectureModule = await withTimeout(import(`./runtime/architecture-v82.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Arquitetura do aplicativo");
    ...
    await withTimeout(import(`./ninou-core-v82.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Núcleo do aplicativo");

    const layerResults = await Promise.allSettled([
      withTimeout(import(`./ninou-ux-v82.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Experiência de uso"),
      withTimeout(import(`./ninou-consistency-v82.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Consistência visual"),
      withTimeout(import(`./ninou-stability-v82.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Estabilidade"),
      withTimeout(import(`./runtime/diagnostics-v82.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Diagnóstico"),
      withTimeout(import(`./runtime/visual-guard-v82.0.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Guarda visual"),
    ]);
