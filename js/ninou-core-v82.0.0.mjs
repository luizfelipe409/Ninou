...
const NINOU_RUNTIME_VERSION = "82.0.3";
...
const NINOU_FAMILY_SCOPE_VERSION = "82.0.3-premium-consolidated";
...
function prepareVisibleContextForAccount(user = cloudUser) {
  ...
  /*
    v82.0.3 — retomada sem tela fantasma:
  ...
  try {
    ...
    if (inviteResult) {
      inviteResult.textContent = error?.code === "permission-denied"
        ? "Sem permissão para criar convite. Publique as regras Firestore da v82.0.3 e confirme que está logado com luizfelipe.dasilva@gmail.com."
  ...
  try {
    ...
    if (!options.silent && loginHelper) {
      loginHelper.textContent = error?.code === "permission-denied"
        ? "Sem permissão para aceitar convite. Publique as regras Firestore da v82.0.3 e confirme se o convite é para este e-mail."
        : getFirebaseErrorMessage(error);
    }
    return false;
  } finally {
...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("/sw.js?v=82.0.3", { updateViaCache: "none" }).then((registration) => {
