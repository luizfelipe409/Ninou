...
const ADMIN_STYLESHEET_HREF = "./styles/admin-web-v82.1.7.css?v=82.1.13";
const ADMIN_RUNTIME_HREF = "./ninou-admin-web-v82.1.7.mjs?v=82.1.13";
...
function showAdminRuntimeFallback(error = null) {
  const panelRoot = document.querySelector("#adminWebPortal");
...
async function activateGlobalAdminWebPortal(user = cloudUser) {
  ...
  document.body.dataset.adminPortalOpen = "true";
  if (panelRoot) {
    panelRoot.hidden = false;
    panelRoot.setAttribute("aria-hidden", "false");
    panelRoot.dataset.adminPortalState = "loading";
  }
  window.dispatchEvent(new CustomEvent("ninou:admin-portal-opening", { detail: { email: user?.email || "" } }));
  showScreen("profile");
  ...
  if (ready && panelRoot) {
    panelRoot.hidden = false;
    panelRoot.setAttribute("aria-hidden", "false");
    panelRoot.dataset.adminPortalReady = "true";
    panelRoot.dataset.adminPortalState = "ready";
    window.dispatchEvent(new CustomEvent("ninou:admin-portal-ready", { detail: { email: user?.email || "" } }));
  }
  return ready;
}
...
const adminInvitePanel = document.querySelector("#adminWebPortal");
...
const NINOU_RUNTIME_VERSION = "82.1.13";
...
const NINOU_FAMILY_SCOPE_VERSION = "82.1.13-web-mobile-experience";
...
function setAuthAccessLoading(value, message = "Validando acesso familiar...") {
  ...
  const keepStableSurface = Boolean(cloudUser && hasFamilyAccess() && familyBootstrapReady);
  document.body.classList.toggle("auth-validating", authAccessLoading);
  document.body.classList.toggle("sync-bootstrap", authAccessLoading && Boolean(cloudUser) && !keepStableSurface);
  document.body.classList.toggle("auth-refreshing-stable", authAccessLoading && keepStableSurface);
  if (authAccessLoading && loginHelper) loginHelper.textContent = message;
  if (authAccessLoading && window.__NINOU_APP_READY__) {
    window.NinouLoading?.show?.(message || "Atualizando sua rotina…", { resume: true });
  }
  if (!authAccessLoading) {
    window.dispatchEvent(new CustomEvent("ninou:auth-ready", { detail: { user: cloudUser?.email || "", familyReady: familyBootstrapReady } }));
    if (document.documentElement.dataset.ninouInitialPaint) {
      window.NinouLoading?.hide?.({ reason: "auth-ready" });
    }
  }
  renderAuthControls();
}
...
function getProfileAccessState() {
  if (!authStateResolved) return "booting";
...
function renderLoggedOutProfileShell() {
  const loggedOut = authStateResolved && !isLoggedIn();
...
function applyGuestInteractionLock() {
  const locked = authStateResolved && !isLoggedIn();
  const stableRefresh = Boolean(cloudUser && hasFamilyAccess() && familyBootstrapReady && authAccessLoading);
  document.body.classList.toggle("guest-locked", locked);
  document.body.classList.toggle("auth-validating", authAccessLoading);
  document.body.classList.toggle("sync-bootstrap", authAccessLoading && Boolean(cloudUser) && !stableRefresh);
  document.body.classList.toggle("auth-refreshing-stable", stableRefresh);
...
function renderGuestPremiumContent() {
  ...
  if (!authStateResolved || authAccessLoading || isLoggedIn() || cloudUser) return;
...
function normalizeLoggedProfileCards() {
  ...
  const authorized = hasFamilyAccess();
  const appAdmin = isGlobalAppAdmin();
  const stableRefresh = Boolean(authorized && familyBootstrapReady && authAccessLoading);
  const ready = Boolean(authorized && (!authAccessLoading || stableRefresh) && (!appAdmin || Boolean(window.__ninouAdminFamilyDataOpen)));
...
function prepareVisibleContextForAccount(user = cloudUser) {
  ...
  const previousOwner = getVisibleDataOwnerEmail() || normalizeEmail(localStorage.getItem(storageKeys.email) || "");
  const sameOwner = Boolean(previousOwner && previousOwner === email);

  if (previousOwner && !sameOwner) {
    saveCurrentVisibleContextForOwner(previousOwner);
  }

  /*
    v82.0.0 — retomada sem tela fantasma:
  ...
  */
  if (sameOwner) {
    ...
    selectedDiaryDay = selectedDiaryDay || getDayStart();
    loadedStateDayId = loadedStateDayId || getCurrentDayId();
    familyBootstrapReady = Boolean(hasFamilyAccess());
    updateProfilePhoto(getBabyAvatarDataUrl(babyProfile.avatar || pendingBabyAvatar));
  ...
  const restored = restoreVisibleContextForOwner(email);
  if (!restored) {
    clearGenericVisibleContext();
  ...
  try {
    ...
    if (inviteResult) {
      inviteResult.textContent = error?.code === "permission-denied"
        ? "Sem permissão para criar convite. Publique as regras Firestore da v82.0.0 e confirme que está logado com luizfelipe.dasilva@gmail.com."
  ...
  try {
    ...
    if (!options.silent && loginHelper) {
      loginHelper.textContent = error?.code === "permission-denied"
        ? "Sem permissão para aceitar convite. Publique as regras Firestore da v82.0.0 e confirme se o convite é para este e-mail."
        : getFirebaseErrorMessage(error);
    }
    return false;
  } finally {
...
function updateBodyModeClasses() {
  ...
  const previewOpen = Boolean(window.__ninouAdminFamilyDataOpen);
  const adminPortalOpen = appAdmin && !previewOpen;
  const familySurface = Boolean(isLoggedIn() && hasFamilyAccess() && (!appAdmin || previewOpen));
  const adminPortal = document.querySelector("#adminWebPortal");
  ...
  document.body.classList.toggle("global-admin-mode", adminPortalOpen);
  document.body.classList.toggle("admin-panel-only", adminPortalOpen);
  ...
  document.body.dataset.adminPortalOpen = adminPortalOpen ? "true" : "false";
  renderAdminClients();
}
...
function renderAuthControls() {
  ...
  const appAdmin = isGlobalAppAdmin();
  const stableRefresh = Boolean(authorized && familyBootstrapReady && authAccessLoading);
  const routineAuthorized = authorized && (!authAccessLoading || stableRefresh) && (!appAdmin || Boolean(window.__ninouAdminFamilyDataOpen));
...
async function initFirebaseAuthState() {
  ...
  services.onAuthStateChanged(services.auth, async (user) => {
    ...
    try {
      ...
      const cachedOwner = getVisibleDataOwnerEmail();
      const keepCachedFamily = Boolean(hasFamilyAccess() && cachedOwner === normalizeEmail(user.email || ""));
      familyBootstrapReady = keepCachedFamily;
      if (!keepCachedFamily) document.body.classList.remove("family-ready");
      document.body.classList.remove("sync-bootstrap");
      setAuthAccessLoading(false);
      setSyncStatus(keepCachedFamily ? "pending" : "offline", user.email || "", keepCachedFamily ? "Exibindo os dados deste aparelho até a conexão voltar." : "");
      loginHelper.textContent = keepCachedFamily
...
function showScreen(target) {
  // Navegar nunca pode deixar a folha de registro ou o timer sobre outra tela.
  if (sheet && !sheet.hidden) closeSheet();
  if (orbitClusterSheet && !orbitClusterSheet.hidden) closeOrbitCluster();
  if (breastTimerPanel) breastTimerPanel.hidden = true;
...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("/sw.js?v=82.1.13", { updateViaCache: "none" }).then((registration) => {
