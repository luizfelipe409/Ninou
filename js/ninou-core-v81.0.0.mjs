...
const NINOU_RUNTIME_VERSION = "81.0.0";
...
const NINOU_FAMILY_SCOPE_VERSION = "81.0.0-premium-consolidated";
...
function prepareVisibleContextForAccount(user = cloudUser) {
  ...
  /*
    v81.0.0 — retomada sem tela fantasma:
  ...
  try {
    ...
    if (inviteResult) {
      inviteResult.textContent = error?.code === "permission-denied"
        ? "Sem permissão para criar convite. Publique as regras Firestore da v81.0.0 e confirme que está logado com luizfelipe.dasilva@gmail.com."
  ...
  try {
    ...
    if (!options.silent && loginHelper) {
      loginHelper.textContent = error?.code === "permission-denied"
        ? "Sem permissão para aceitar convite. Publique as regras Firestore da v81.0.0 e confirme se o convite é para este e-mail."
        : getFirebaseErrorMessage(error);
    }
    return false;
  } finally {
...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("/sw.js?v=81.0.0", { updateViaCache: "none" }).then((registration) => {
