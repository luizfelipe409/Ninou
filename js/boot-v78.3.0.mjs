const NINOU_VERSION = "78.3.0";
const LEGACY_REPAIR_KEY = "ninou:pwa-legacy-cleanup:v78.3.0";
...
async function waitForStyleSheets() {
  const links = [...document.querySelectorAll('link[rel="stylesheet"][href*="78.3.0"]')];
...
async function cleanLegacyRuntimeOnce() {
  ...
  if ("caches" in window) {
    try {
      ...
      const legacy = names.filter((name) => name.toLowerCase().includes("ninou") && !name.includes("78-3-0"));
...
async function bootNinou() {
  ...
  window.__NINOU_BOOT_PROMISE__ = (async () => {
    ...
    const architectureModule = await withTimeout(import(`./runtime/architecture-v78.3.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Arquitetura do aplicativo");
    ...
    await withTimeout(import(`./ninou-core-v78.3.0.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Núcleo do aplicativo");
    await withTimeout(Promise.all([
      import(`./ninou-ux-v78.3.0.mjs?v=${NINOU_VERSION}`),
      import(`./ninou-consistency-v78.3.0.mjs?v=${NINOU_VERSION}`),
      import(`./ninou-stability-v78.3.0.mjs?v=${NINOU_VERSION}`),
      import(`./runtime/diagnostics-v78.3.0.mjs?v=${NINOU_VERSION}`),
