const NINOU_VERSION = "83.0.2";
const LEGACY_REPAIR_KEY = "ninou:pwa-legacy-cleanup:v83.0.2";
...
async function waitForStyleSheets() {
  const links = [...document.querySelectorAll('link[rel="stylesheet"][data-ninou-style]')];
...
async function cleanLegacyRuntimeOnce() {
  ...
  if ("caches" in window) {
    try {
      ...
      const legacy = names.filter((name) => name.toLowerCase().includes("ninou") && !name.includes(NINOU_VERSION));
...
async function bootNinou() {
  ...
  window.__NINOU_BOOT_PROMISE__ = (async () => {
    ...
    try {
      const architectureModule = await withTimeout(import(`./runtime/architecture.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Arquitetura do aplicativo");
    ...
    await withTimeout(import(`./app-core.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Núcleo do aplicativo");

    const layerResults = await Promise.allSettled([
      withTimeout(import(`./ui/app-shell.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Shell da aplicação"),
      withTimeout(import(`./ui/ux-runtime.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Experiência de uso"),
      withTimeout(import(`./ui/consistency-runtime.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Consistência visual"),
      withTimeout(import(`./runtime/stability.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Estabilidade"),
      withTimeout(import(`./runtime/diagnostics.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Diagnóstico"),
      withTimeout(import(`./runtime/visual-guard.mjs?v=${NINOU_VERSION}`), MODULE_TIMEOUT_MS, "Guarda visual"),
    ]);
