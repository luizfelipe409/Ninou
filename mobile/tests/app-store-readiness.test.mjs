...
assert.equal(appConfig.expo.version, '82.1.8');
assert.equal(packageConfig.version, '82.1.8');
assert.equal(appConfig.expo.ios.bundleIdentifier, 'com.ninou.app');
assert.equal(appConfig.expo.ios.buildNumber, '85');
assert.equal(appConfig.expo.android.versionCode, 85);
...
assert.match(project, /MARKETING_VERSION = 82\.1\.8;/);
