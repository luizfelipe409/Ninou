...
function formatEventInterval(event: RoutineEvent) {
...
export default function DiaryScreen() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<RoutineEvent | null>(null);
  ...
  useEffect(() => {
    if (!canWrite || freeform === dayState.dayNotes) return;
    const timer = setTimeout(() => updateDayNotes(selectedDayId, freeform), 700);
    return () => clearTimeout(timer);
  }, [canWrite, dayState.dayNotes, freeform, selectedDayId, updateDayNotes]);
  ...
  const saveEdit = () => {
    ...
    const rawEnd = editEnd ? timeOnDay(selectedDayId, editEnd) : start;
    const end = editEnd ? resolveRoutineIntervalEnd(editing.type, start, rawEnd) ?? start : start;
    if (editEnd && end <= start && (editing.type === 'sono' || editing.type === 'dormir')) {
  ...
  const confirmDelete = (event: RoutineEvent) => { if (canWrite) setDeleteTarget(event); };
  const editCrossesMidnight = Boolean(editing && editEnd && (editing.type === 'sono' || editing.type === 'dormir') && timeOnDay(selectedDayId, editEnd) < timeOnDay(selectedDayId, editStart));
  ...
  const saveEpisode = (icon: string, text: string) => { if (canWrite) addNoteEpisode(selectedDayId, { icon, text, time: Date.now(), caregiver: caregiverLabel }); };
    ...
    <NinouScreen eyebrow={displayDate(selectedDayId)} title="Diário" subtitle="Registros, acontecimentos e notas compartilhadas pela família.">
      ...
      <Modal visible={Boolean(deleteTarget)} transparent animationType="slide" onRequestClose={() => setDeleteTarget(null)}>
        <View style={styles.deleteBackdrop}>
          <Pressable accessibilityLabel="Fechar confirmação" style={StyleSheet.absoluteFill} onPress={() => setDeleteTarget(null)} />
          <View accessibilityViewIsModal style={[styles.deleteDialog, { backgroundColor: colors.surface, borderColor: colors.border, shadowColor: isDark ? '#000' : colors.primary }]}>
            <View style={[styles.deleteHandle, { backgroundColor: colors.border }]} />
            <View style={styles.deleteHeader}>
              <View style={[styles.deleteIcon, { backgroundColor: `${colors.danger}14`, borderColor: `${colors.danger}38` }]}><Ionicons name="trash-bin-outline" size={27} color={colors.danger} /></View>
              <View style={styles.deleteHeaderCopy}><Text style={[styles.deleteKicker, { color: colors.danger }]}>ALTERAÇÃO NO DIÁRIO</Text><Text style={[styles.deleteTitle, { color: colors.text }]}>Remover este registro?</Text></View>
              <Pressable accessibilityLabel="Fechar" onPress={() => setDeleteTarget(null)} style={[styles.deleteClose, { backgroundColor: colors.surfaceElevated }]}><Ionicons name="close" size={20} color={colors.text} /></Pressable>
            </View>
            {deleteTarget ? <View style={[styles.deletePreview, { backgroundColor: colors.surfaceElevated, borderColor: colors.border }]}><ActionArt type={deleteTarget.type} size={52} /><View style={styles.deletePreviewCopy}><Text style={[styles.deletePreviewTitle, { color: colors.text }]}>{recordConfig[deleteTarget.type].title}</Text><Text style={[styles.deletePreviewMeta, { color: colors.primary }]}>{formatEventInterval(deleteTarget)}</Text>{deleteTarget.detail ? <Text numberOfLines={2} style={[styles.deletePreviewDetail, { color: colors.textMuted }]}>{deleteTarget.detail}</Text> : null}</View></View> : null}
            <View style={[styles.deleteNotice, { backgroundColor: `${colors.danger}0D` }]}><Ionicons name="sync-outline" size={18} color={colors.danger} /><Text style={[styles.deleteWarning, { color: colors.textMuted }]}>A remoção será atualizada em todos os aparelhos da família e não poderá ser desfeita.</Text></View>
            <Pressable onPress={() => { if (deleteTarget) deleteEvent(selectedDayId, deleteTarget.id); setDeleteTarget(null); }} style={[styles.deleteConfirm, { backgroundColor: colors.danger }]}><Ionicons name="trash" size={18} color="#FFF" /><Text style={styles.deleteConfirmText}>Remover registro</Text></Pressable>
            <Pressable onPress={() => setDeleteTarget(null)} style={styles.deleteCancel}><Text style={[styles.deleteCancelText, { color: colors.textMuted }]}>Voltar ao diário</Text></Pressable>
          </View>
        </View>
      </Modal>
