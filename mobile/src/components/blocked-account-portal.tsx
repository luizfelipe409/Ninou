...
export function BlockedAccountPortal() {
  ...
  const { user, accountStatus, signOut } = useNinouAuth();
  const deletionPending = ['deletion_requested', 'deletion-pending', 'pending_deletion'].includes(accountStatus);
  const title = deletionPending ? 'Exclusão em processamento' : 'Acesso suspenso';
  const body = deletionPending
    ? `A exclusão da conta ${user?.email || ''} foi iniciada. O acesso permanece desativado durante o processamento da solicitação.`
    : `A conta ${user?.email || ''} foi temporariamente bloqueada pela administração. Seus dados permanecem preservados.`;
  const hint = deletionPending
    ? 'Dados compartilhados com uma família podem ser preservados para os demais responsáveis até a conclusão do processo.'
    : 'Entre em contato com o suporte Ninou para revisar o acesso.';
  return <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}><NinouBackground intense /><View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.icon, { backgroundColor: `${colors.danger}14` }]}><Ionicons name={deletionPending ? 'trash-outline' : 'lock-closed-outline'} size={31} color={colors.danger} /></View><Text style={[styles.title, { color: colors.text }]}>{title}</Text><Text style={[styles.body, { color: colors.textMuted }]}>{body}</Text><Text style={[styles.hint, { color: colors.textMuted }]}>{hint}</Text><Pressable onPress={() => void signOut()} style={[styles.button, { backgroundColor: colors.primary }]}><Ionicons name="log-out-outline" size={18} color="#FFFFFF" /><Text style={styles.buttonText}>Voltar para o login</Text></Pressable></View></SafeAreaView>;
}
