...
const options = ['Pai', 'Mãe', 'Responsável', 'Avó', 'Avô', 'Babá', 'Cuidador(a)', 'Tia', 'Tio'];
...
export function RelationPicker({ label = 'Relação com o bebê', value, onChange, disabled = false }: { label?: string; value: string; onChange: (value: string) => void; disabled?: boolean }) {
  ...
  const selectedKnown = options.includes(value);
  ...
  return (
    <View style={styles.field}>
      ...
      <Pressable
        ...
        <Text style={[styles.value, { color: value ? colors.text : colors.textMuted }]}>{value || 'Escolher relação'}</Text>
        ...
        <KeyboardAvoidingView style={styles.backdrop} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          ...
          <View style={[styles.sheet, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <ScrollView contentContainerStyle={styles.options} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={[styles.customCard, { backgroundColor: colors.surfaceElevated, borderColor: !selectedKnown && value ? colors.primary : colors.border }]}>
...
const styles = StyleSheet.create({
  ...
  sheet: { width: '100%', maxWidth: 540, maxHeight: '88%', alignSelf: 'center', borderTopLeftRadius: 30, borderTopRightRadius: 30, borderWidth: StyleSheet.hairlineWidth, padding: 18 },
  ...
  options: { paddingTop: 16, paddingBottom: 28, gap: 8 },
  option: { minHeight: 54, borderRadius: 17, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 11 },
