...
const ROLE_LABEL: Record<string, string> = { owner: 'Responsável', admin_familiar: 'Administrador familiar', cuidador: 'Cuidador', caregiver: 'Cuidador', visualizacao: 'Somente leitura' };
...
export function GlobalAdminPortal() {
  ...
  const ask = (next: Confirmation) => { setConfirmReason(''); setConfirmation(next); };
  const confirm = async () => {
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
      ...
      <FamilyModal key={selectedFamily?.id || 'no-family'} family={selectedFamily} close={() => setSelectedFamilyId('')} user={user!} runAction={runAction} ask={ask} busy={busy} />
...
function FamilyModal({ family, close, user, runAction, ask, busy }: { family: AdminFamily | null; close: () => void; user: NonNullable<ReturnType<typeof useNinouAuth>['user']>; runAction: (action: () => Promise<void>, success: string) => Promise<void>; ask: (v: Confirmation) => void; busy: boolean }) {
  const { colors } = useNinouTheme();
  ...
  return <Modal visible animationType="slide" onRequestClose={close}><SafeAreaView style={[styles.modalSafe, { backgroundColor: colors.background }]}><View style={[styles.modalHeader, { borderBottomColor: colors.border }]}><Pressable onPress={close} style={[styles.iconButton, { backgroundColor: colors.surface, borderColor: colors.border }]}><Ionicons name="close" size={22} color={colors.text} /></Pressable><View style={styles.modalHeaderCopy}><Text numberOfLines={1} style={[styles.modalTitle, { color: colors.text }]}>{family.name}</Text><Text style={[styles.rowSubtitle, { color: colors.textMuted }]}>{family.id}</Text></View><Status value={family.status} /></View>
    <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
      ...
      <Panel title="Assinatura e entitlement" icon="card-outline"><View style={styles.filterRow}>{(['trial', 'premium', 'courtesy', 'suspended'] as AdminPlan[]).map((value) => <Chip key={value} selected={plan === value} label={PLAN_LABEL[value]} onPress={() => setPlan(value)} />)}</View><Field label="Duração em dias" value={days} onChange={setDays} keyboardType="number-pad" /><PrimaryButton label="Atualizar plano" icon="sparkles-outline" disabled={busy} onPress={() => ask({ title: 'Atualizar assinatura', message: `Aplicar o plano ${PLAN_LABEL[plan]} por ${days || '30'} dias?`, familyId: family.id, reasonRequired: true, run: async () => adminUpdateSubscription(user, family, { plan, days: Number(days) || 30 }) })} /></Panel>
...
function MemberRow({ family, member, user, ask }: { family: AdminFamily; member: AdminMember; user: NonNullable<ReturnType<typeof useNinouAuth>['user']>; ask: (v: Confirmation) => void }) {
  const { colors } = useNinouTheme();
  return <View style={[styles.memberRow, { borderBottomColor: colors.border }]}><View style={styles.titleLine}><View style={styles.rowCopy}><Text style={[styles.rowTitle, { color: colors.text }]}>{member.name || member.email || member.uid}</Text><Text style={[styles.rowSubtitle, { color: colors.textMuted }]}>{member.email} · {ROLE_LABEL[member.role] || member.role}</Text></View><Status value={member.status} /></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.memberActions}>{member.role !== 'owner' ? <Pressable onPress={() => ask({ title: 'Transferir responsabilidade', message: `${member.email} passará a ser o responsável principal da família.`, familyId: family.id, reasonRequired: true, run: async () => adminTransferOwnership(user, family, member) })} style={[styles.inlineAction, { borderColor: colors.primary }]}><Text style={[styles.inlineActionText, { color: colors.primary }]}>Tornar responsável</Text></Pressable> : null}{member.role !== 'owner' ? ['admin_familiar', 'cuidador', 'visualizacao'].map((role) => <Pressable key={role} onPress={() => ask({ title: 'Alterar permissão', message: `Alterar ${member.email} para ${ROLE_LABEL[role]}?`, familyId: family.id, reasonRequired: true, run: async () => adminUpdateMember(user, family, member, { role }) })} style={[styles.inlineAction, { borderColor: member.role === role ? colors.accent : colors.border }]}><Text style={[styles.inlineActionText, { color: member.role === role ? colors.accent : colors.textMuted }]}>{ROLE_LABEL[role]}</Text></Pressable>) : null}<Pressable onPress={() => ask({ title: member.status === 'blocked' ? 'Reativar membro' : 'Remover acesso', message: `Alterar o acesso de ${member.email}?`, familyId: family.id, destructive: member.status !== 'blocked', reasonRequired: true, run: async () => adminUpdateMember(user, family, member, { status: member.status === 'blocked' ? 'active' : 'blocked' }) })} style={[styles.inlineAction, { borderColor: colors.danger }]}><Text style={[styles.inlineActionText, { color: colors.danger }]}>{member.status === 'blocked' ? 'Reativar' : 'Remover'}</Text></Pressable></ScrollView></View>;
}
...
const styles = StyleSheet.create<Record<string, any>>({
  ...
  memberRow: { paddingVertical: 11, borderBottomWidth: StyleSheet.hairlineWidth }, memberActions: { paddingTop: 9, gap: 6 }, inlineAction: { minHeight: 31, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: 9, alignItems: 'center', justifyContent: 'center' }, inlineActionText: { fontSize: 8.5, fontWeight: '800' }, inviteRow: { minHeight: 59, marginTop: 10, paddingTop: 10, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: 'row', alignItems: 'center', gap: 8 }, dayRow: { minHeight: 51, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, eventDots: { flexDirection: 'row', gap: 3 }, eventDot: { width: 7, height: 7, borderRadius: 4 }, dualActions: { marginTop: 9, flexDirection: 'row', gap: 7 }, smallAction: { flex: 1, minHeight: 43, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }, smallActionText: { fontSize: 8.8, fontWeight: '850' },
